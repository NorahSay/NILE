...
/*		COLLECT DATA ARRAY: WEIGHT VS TIME
 * 	INPUT: - Load Cell #0-3,
 * 			 -'samples' array structure,
 * 			 - number of desired points,
 * 			 - desired time interval between samples (in ms)
*/
	  sample_num_points = 50;
	  weight_sample_t samples[num_points];
	  collect_weight_data(&loadcell[0], samples, sample_num_points, 500);
	  collect_weight_data(&loadcell[1], samples, sample_num_points, 500);
	  collect_weight_data(&loadcell[2], samples, sample_num_points, 500);
	  collect_weight_data(&loadcell[3], samples, sample_num_points, 500);

/*
 * 	LOG SAMPLES
 */

	  log_samples_csv(samples, sample_num_points);
...
void log_samples_csv(weight_sample_t *data_array, uint16_t sample_num_points)
{
	printf("timestamp_ms,weight_g\r\n");

	for (uint16_t i = 0; i < num_points; i++) {
	    printf("%lu,%.2f\r\n", data_array[i].timestamp_ms, data_array[i].weight_g);
	}
